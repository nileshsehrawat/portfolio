...
const Works = () => {
  ...
  return (
    <section
      className="flex min-h-screen flex-col gap-8 py-8 sm:py-16 lg:gap-16"
      id="work"
    >
      <AnimatedHeader
        lineColor="bg-dark"
      ...
      >
        {projectsData.map((project, index) => (
          ...
          >
            {/* overlay */}
            <div
              className="clip-path absolute inset-0 hidden bg-dark duration-300 md:block"
              ref={(el) => {
            ...
            <a
              ...
              target="_blank"
            >
              <h2 className="text-2xl leading-none lg:text-3xl">
                {project.name}
              </h2>
              <ArrowUpRightIcon />
            </a>
        ...
        >
          {currentIndex !== null && (
            <>
              ...
              <Desc
                className="p-4"
                text={projectsData[currentIndex].description}
              />
            </>
          )}
        </div>
...
const Desc = ({ text, className }) => (
  <p className={`text-base leading-relaxed md:text-lg ${className}`}>{text}</p>
);
